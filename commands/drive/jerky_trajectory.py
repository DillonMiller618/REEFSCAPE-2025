...
class SwerveTrajectory(JerkyTrajectory):
    ...
    def initialize(self):
        ...
        import math
        from constants import MECH, AUTO
        ...
        config.setKinematics(MECH.kDriveKinematics)
        ...
        thetaController = ProfiledPIDControllerRadians(
            AUTO.kPThetaController,
            0,
            0,
            AUTO.kThetaControllerConstraints,
        )
        thetaController.enableContinuousInput(-math.pi, math.pi)

        driveController = HolonomicDriveController(
            PIDController(AUTO.kPXController, 0, 0),
            PIDController(AUTO.kPXController, 0, 0),
            thetaController,
        )
        ...
        swerveControllerCommand = commands2.SwerveControllerCommand(
            trajectory,
            self.drivetrain.pose,  # Functional interface to feed supplier
            MECH.kDriveKinematics,
            driveController, 
            self.drivetrain.desire_module_states, #check if this is the "right" method
            (self.drivetrain,),
        )

        # Run path following command, then stop at the end
        stop = InstantCommand(
            lambda: self.drivetrain.drive(0, 0, False, False),
            self.drivetrain,
        )
