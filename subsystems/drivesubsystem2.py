...
class DriveSubsystem(commands2.Subsystem):
    ...
    def get_chassis_speeds(self):
        ...
        return DriveConstants.kDriveKinematics.toChassisSpeeds((self.m_FL.get_state_onboard(),
    ...
    def drive_2ok_clt(self, x_speed: float, y_speed: float, rot: float, field_relative: bool) -> None:
        ...
        if rot != 0:
            ...
            self.clt_reset = True
        elif -0.5 * DriveConstants.kMaxSpeedMetersPerSecond <= x_speed <= 0.5 * DriveConstants.kMaxSpeedMetersPerSecond and \
                -0.5 * DriveConstants.kMaxSpeedMetersPerSecond <= y_speed <= 0.5 * DriveConstants.kMaxSpeedMetersPerSecond:
            self.drive_2ok(x_speed, y_speed, rot * DriveConstants.kMaxAngularSpeed, field_relative)
            self.clt_reset = True
        else:
            if self.clt_reset:
    ...
    def drive(self, x_speed: float, y_speed: float, rot: float, field_relative: bool) -> None:
        ...
        # If in robot relative mode, get swerve module states.
            ...
            swerve_module_states = DriveConstants.kDriveKinematics.toSwerveModuleStates(ChassisSpeeds(-x_speed,
                                                                                                  -y_speed, -rot))
    ...
    def get_field_relative_velocity(self) -> Union[float, float]:
    ...
    def get_field_relative_acceleration(self, new_speed, old_speed, time: float) -> Union[float, float, float]:
    ...
    def get_current_draw_all_modules(self) -> Union[Union[float, float], Union[float, float], Union[float, float], Union[float, float]]:
